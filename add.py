...
class ImgOccAdd(object):
        ...
        if not image_path:
            if self.origin == "addcards":
                ...
                if askUser(_("Select multiple images for batch processing?"), title=_("Multi-Image Selection")):
                    image_paths = self.getNewImage(parent=self.ed.parentWindow, multi_select=True)
                    if not image_paths:
                        return False
                    return self.callImgOccEditMulti(image_paths)
                else:
    ...
    def callImgOccEditMulti(self, image_paths):
        """Handle multiple images by calling callImgOccEdit for each"""
        if not image_paths:
            return False
            
        # Initialize opref for multi-image workflow
        note = self.ed.note
        if note:
            self.setPreservedAttrs(note)
        else:
            # Initialize opref with default values for new workflow
            self.opref["tags"] = []
            if self.origin == "addcards":
                self.opref["did"] = self.ed.parentWindow.deckChooser.selectedId()
            else:
                self.opref["did"] = mw.col.conf['curDeck']
            
        # Get dimensions of first image to initialize editor
        try:
            ...
            showWarning(
                _(
                    "<b>Unsupported image</b> in file <i>{image_path}</i>:"
                    "<br><br>{error}"
                ).format(image_path=image_paths[0], error=str(e))
            )
            return False
        ...
        # Pass all images to the editor
        if hasattr(self, 'imgoccedit') and self.imgoccedit:
            self.imgoccedit.setImageList(image_paths)
    ...
    def getNewImage(self, parent=None, noclip=False, multi_select=False):
        ...
        if clip and clip.mimeData().imageData():
            ...
            if os.stat(image_path).st_size == 0:
                ...
                return self.getNewImage(noclip=True, multi_select=multi_select)
            else:
                return [str(image_path)] if multi_select else str(image_path)
        ...
        if multi_select:
            image_paths = QFileDialog.getOpenFileNames(
                parent,
                _("Select Images"),
                prev_image_dir,
        ...
        if multi_select:
            if not image_paths:
                return None
            # Validate all files exist and are valid images
            valid_paths = []
            invalid_files = []
            for path in image_paths:
                if os.path.isfile(path):
                    try:
                        # Quick validation by checking if we can get dimensions
            ...
            if invalid_files:
                ...
                showWarning(
                    _("The following files are invalid or not supported:\n\n{files}\n\n"
                      "They will be skipped.").format(files="\n".join(invalid_files))
                )
            
            if valid_paths:
                self.lconf["dir"] = os.path.dirname(valid_paths[0])
                return valid_paths
            else:
                return None
        else:
    ...
    def _onAddNotesButton(self, choice, close, svg):
        ...
        if close:
            dialog.close()
        elif hasattr(dialog, 'multi_image_mode') and dialog.multi_image_mode:
            # Handle multi-image progression
            dialog.handlePostSaveAction()
